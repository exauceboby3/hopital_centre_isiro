...
export function LabCatalogEditor() {
  const { user } = useAuth();
  const canManagePrice = hasAnyRole(user, ['SUPER_ADMIN', 'ADMIN']);
  ...
  const save = async (event: FormEvent) => {
    ...
    if (!editing && !canManagePrice) {
      setError(
        'Sélectionnez un examen existant à configurer. La création tarifaire est réservée à l’administration.',
      );
      return;
    }
    ...
    try {
      await api(editing ? `/laboratory/exams/catalog/${editing.id}` : '/laboratory/exams/catalog', {
        ...
        body: JSON.stringify({
          ...
          ...(canManagePrice ? { price: Number(form.price) } : {}),
                    ...
                    return (
                      ...
                      >
                        <span>
                          ...
                          <small>
                            {service.code} · {service.category || 'Sans catégorie'}
                          </small>
                          <small>
                            {template.fields.length} rubrique(s)
                            {template.specimenType ? ` · ${template.specimenType}` : ''}
                          </small>
                        </span>
                        <Pencil size={15} />
                      </button>
            ...
            <form className="lab-catalog-form" onSubmit={save}>
              <div className="form-grid">
                ...
                <label className="field">
                  ...
                  <input
                    maxLength={150}
                    placeholder="Sang total, sérum, urine…"
                    value={form.specimenType}
                    onChange={(event) => setForm({ ...form, specimenType: event.target.value })}
                  />
                </label>
                <label className="field full">
                  <span>Méthode</span>
                  <input
                    maxLength={200}
                    placeholder="Méthode analytique ou automate"
                    value={form.method}
                ...
                <label className="check-row full">
                  ...
                  />
                  <span>Examen actif et disponible à la prescription</span>
                </label>
              </div>
              ...
              <div className="lab-result-fields-heading">
                <div>
                  ...
                  <small>
                    Ces unités et références seront reprises automatiquement lors de la saisie.
                  </small>
                </div>
                <button
                  type="button"
                ...
                >
                  <Plus size={15} /> Ajouter une rubrique
                </button>
              </div>
                    ...
                    <div className="form-grid">
                        ...
                        <input
                          ...
                          onChange={(event) =>
                            setField(index, {
                              ...
                              key: field.key || normalizeKey(event.target.value),
                            })
                          }
                        />
                      </label>
                      <label className="field">
                        ...
                        <input
                          ...
                          value={field.key}
                          onChange={(event) =>
                            setField(index, { key: normalizeKey(event.target.value) })
                          }
                        />
                      </label>
                      <label className="field">
                        ...
                        <select
                          value={field.type}
                          onChange={(event) =>
                            setField(index, { type: event.target.value as ResultField['type'] })
                          }
                        >
                      ...
                      <label className="field full">
                        ...
                        <input
                          maxLength={120}
                          placeholder="Ex. Adulte : 12–17"
                          value={field.reference ?? ''}
                          onChange={(event) => setField(index, { reference: event.target.value })}
                        />
                      </label>
                      {field.type === 'SELECT' && (
                        <label className="field full">
                          ...
                          <input
                            ...
                            onChange={(event) =>
                              setField(index, { options: event.target.value.split(',') })
                            }
                          />
                        </label>
                      )}
                      <label className="check-row">
                        ...
                        />
                        <span>Rubrique obligatoire</span>
                      </label>
                    </div>
                    <button
                      type="button"
                      ...
                      disabled={form.resultFields.length === 1}
                      onClick={() =>
                        setForm((current) => ({
                          ...current,
                          resultFields: current.resultFields.filter(
              ...
              <div className="modal-actions">
                ...
                <button
                  ...
                  disabled={saving || (!editing && !canManagePrice)}
                >
                  {saving && <Activity className="spin" size={16} />}
                  {editing ? 'Mettre à jour l’examen' : 'Ajouter l’examen'}
                </button>
              </div>
            </form>
          </div>
