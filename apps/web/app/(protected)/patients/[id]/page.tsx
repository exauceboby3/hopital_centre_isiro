...
export default function PatientRecordPage() {
  ...
  const canDiscuss = hasAnyRole(user, [
    'SUPER_ADMIN',
    'ADMIN',
    'DOCTOR',
    'SURGEON',
  ...
  const load = useCallback(async () => {
    ...
    setError('');
    try {
      const [details, timeline, amendmentRows, discussionRows] = await Promise.all([
        api<PatientDetails>(`/patients/${patientId}`),
        api<History>(`/patients/${patientId}/history`),
        canAmend ? api<Amendment[]>(`/patients/${patientId}/amendments`) : Promise.resolve([]),
        canDiscuss
          ? api<EmergencyDiscussion[]>(`/alerts/patient/${patientId}`)
          : Promise.resolve([]),
      ]);
      setPatient(details);
      setHistory(timeline);
  ...
  }, [canAmend, canDiscuss, patientId]);

  const addDiscussionComment = async (alertId: string) => {
    const value = comment[alertId]?.trim();
    if (!value) return;
    setSaving(true);
    setError('');
    try {
      await api(`/alerts/${alertId}/comments`, {
        method: 'POST',
        body: JSON.stringify({ comment: value }),
      });
      setComment((current) => ({ ...current, [alertId]: '' }));
      await load();
    } catch (reason) {
      setError(reason instanceof Error ? reason.message : 'Commentaire impossible.');
    } finally {
      setSaving(false);
    }
  };
  ...
  if (loading)
    return (
      <section className="panel empty-state">
        <Activity className="spin" /> Ouverture du dossier médical…
      </section>
    );
  if (!patient || !history)
    return <section className="panel alert error">{error || 'Dossier introuvable.'}</section>;

  return (
    <div className="patient-record-shell">
      <header className="patient-record-header">
        <Link className="icon-button" href="/patients" aria-label="Retour">
          <ArrowLeft />
        </Link>
        ...
        <div className="patient-record-identity">
          ...
          <p>
            {patient.medicalRecordNumber} · {ageOf(patient.dateOfBirth)} ·{' '}
        ...
        <div className="patient-record-alerts">
          <span
            className={
              history.paymentClearance.inOrder
                ? 'payment-clearance-ok'
                : 'payment-clearance-warning'
            }
          >
            {history.paymentClearance.inOrder ? 'Paiement en ordre' : 'Paiement à régulariser'}
          </span>
          <span>
            <HeartPulse size={16} /> Groupe {patient.bloodType || 'inconnu'}
          </span>
          <span>
            <ShieldCheck size={16} /> Accès journalisé
          </span>
        </div>
      </header>
        ...
        <div className="patient-record-grid">
          <section className="panel patient-summary-card">
            ...
            <dl>
              <div>
                <dt>Téléphone</dt>
                <dd>{patient.phone || 'Non renseigné'}</dd>
              </div>
              <div>
                <dt>Adresse</dt>
                <dd>{patient.address || 'Non renseignée'}</dd>
              </div>
              <div>
                <dt>Contact d’urgence</dt>
                <dd>{patient.emergencyContact || 'Non renseigné'}</dd>
              </div>
              <div>
                <dt>Date de naissance</dt>
                <dd>
                  {patient.dateOfBirth
                    ? new Date(patient.dateOfBirth).toLocaleDateString('fr-CD')
                    : 'Non renseignée'}
                </dd>
              </div>
            </dl>
          ...
          <section className="panel">
            <div className="panel-heading">
              <div>
                <span className="eyebrow">Résumé clinique</span>
          ...
          <section className="panel patient-latest-events">
            <div className="panel-heading">
              <div>
                <span className="eyebrow">Dernières actions</span>
            ...
            {latest.map((entry) => (
              <article key={`${entry.kind}-${entry.id}`}>
                <div>
                  ...
                  <span>{entry.description || 'Aucun détail'}</span>
                </div>
                <time>{new Date(entry.date).toLocaleString('fr-CD')}</time>
              </article>
            ))}
          </section>
          <section className="panel patient-safety-card">
            <div className="panel-heading">
              <div>
                <span className="eyebrow">Sécurité médicale</span>
                <h2>Règles d’utilisation</h2>
              </div>
              <AlertTriangle />
            </div>
            <p>
              La donnée originale d’une consultation signée reste immuable. Toute correction
              médicale est enregistrée comme avenant avec le médecin, la date, l’ancienne valeur, la
              nouvelle valeur et le motif.
            </p>
          </section>
        </div>
      )}
      ...
      {tab === 'AMENDMENTS' && (
        <div className="patient-amendment-layout">
          {canAmend && (
            <form className="panel" onSubmit={createAmendment}>
              <div className="panel-heading">
                <div>
                  <span className="eyebrow">Traçabilité obligatoire</span>
                  <h2>Ajouter ou corriger une donnée</h2>
                </div>
                <FilePlus2 />
              </div>
              <div className="form-grid">
                ...
                <label className="field full">
                  <span>Consultation liée</span>
                  <select
                    value={form.consultationId}
                    onChange={(event) => setForm({ ...form, consultationId: event.target.value })}
                  >
                    <option value="">Aucune consultation spécifique</option>
                    {patient.consultations?.map((consultation) => (
                      <option key={consultation.id} value={consultation.id}>
                        {new Date(consultation.createdAt).toLocaleDateString('fr-CD')} ·{' '}
                ...
                <label className="field full">
                  ...
                  <textarea
                    ...
                    onChange={(event) => setForm({ ...form, previousValue: event.target.value })}
                  />
                </label>
                <label className="field full">
                  ...
                  <textarea
                    ...
                    rows={3}
                    value={form.newValue}
                    onChange={(event) => setForm({ ...form, newValue: event.target.value })}
                  />
                </label>
                <label className="field full">
                  <span>Argument / motif médical *</span>
                  <textarea
                    required
                    minLength={10}
                    rows={3}
          ...
          <section className="panel">
            <div className="panel-heading">
              <div>
                <span className="eyebrow">Historique immuable</span>
                <h2>Avenants médicaux</h2>
              </div>
              <strong>{amendments.length}</strong>
            </div>
            <div className="patient-amendment-list">
              {amendments.length === 0 ? (
                <p className="muted">Aucune correction médicale enregistrée.</p>
              ) : (
                amendments.map((row) => (
                  <article key={row.id}>
                    <header>
                    ...
                    {row.previousValue && (
                      <p>
                        <b>Ancienne valeur :</b> {row.previousValue}
                      </p>
                    )}
                    <p>
                      <b>Nouvelle valeur :</b> {row.newValue}
                    </p>
                    <p>
                      <b>Motif :</b> {row.reason}
                    </p>
                    <small>
                      {row.author.username} · {new Date(row.createdAt).toLocaleString('fr-CD')}
                    </small>
                  </article>
                ))
              )}
      ...
      {tab === 'DISCUSSIONS' && canDiscuss && (
        <section className="panel">
          <div className="panel-heading">
            <div>
              <span className="eyebrow">Coordination clinique</span>
              <h2>Alertes et discussion sur le patient</h2>
            </div>
            <MessageSquare />
          </div>
          {discussions.length === 0 ? (
            <div className="empty-state">Aucune alerte clinique liée à ce patient.</div>
          ) : (
            discussions.map((alert) => (
              <article className="patient-discussion" key={alert.id}>
                <header>
                  <div>
                    <span className="eyebrow">
                      <Siren size={14} /> {alert.severity} · {alert.status}
                    </span>
                    <strong>{alert.title}</strong>
                  </div>
                  <time>{new Date(alert.createdAt).toLocaleString('fr-CD')}</time>
                </header>
                <p>{alert.message}</p>
                <small>
                  {alert.location || 'Lieu non précisé'} · déclenchée par {alert.createdBy.username}
                </small>
                <div className="patient-discussion-comments">
                  {alert.comments.map((entry) => (
                    <div key={entry.id}>
                      <strong>{entry.author.username}</strong>
                      <p>{entry.comment}</p>
                      <small>{new Date(entry.createdAt).toLocaleString('fr-CD')}</small>
                    </div>
                  ))}
                </div>
                <div className="patient-discussion-compose">
                  <textarea
                    rows={2}
                    value={comment[alert.id] ?? ''}
                    onChange={(event) =>
                      setComment((current) => ({ ...current, [alert.id]: event.target.value }))
                    }
                    placeholder="Ajouter une observation, une décision ou une consigne clinique…"
                  />
                  <button
                    className="primary-button compact"
                    disabled={saving || !comment[alert.id]?.trim()}
                    onClick={() => void addDiscussionComment(alert.id)}
                  >
                    Ajouter
                  </button>
                </div>
