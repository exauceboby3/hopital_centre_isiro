...
export class LaboratoryBatchValidationService {
  ...
  async validate(requestGroupId: string, userId: string) {
    ...
    if (!active.length)
      throw new BadRequestException('Tous les examens de cette demande sont annulés.');
    ...
    if (active.some((exam) => exam.status === ExamStatus.COMPLETED && !exam.result?.trim())) {
      throw new BadRequestException(
        'Chaque examen doit contenir un résultat avant la validation groupée.',
      );
    }
    ...
    return this.prisma.$transaction(
      async (transaction) => {
        ...
        const consultationIds = [
          ...new Set(
            active.map((exam) => exam.consultationId).filter((id): id is string => Boolean(id)),
          ),
        ];
        ...
        const first = exams[0]!;
        const patientName = [
          first.patient.lastName,
          first.patient.postName,
        ...
        const rows = await transaction.examRequest.findMany({
          where: { requestGroupId },
          include: {
            patient: true,
        ...
        return rows.map(stripLabFinancialDetails);
      },
