...
export class HospitalizationsService {
  ...
  async list(status: HospitalizationStatus | undefined, user: AuthenticatedUser) {
    ...
    return rows.map((row) =>
      this.present(row, approvalByHospitalization.get(row.id)?.createdAt, user),
    );
  }
  ...
  async admit(dto: AdmitPatientDto, user: AuthenticatedUser) {
    ...
    return this.prisma.$transaction(
      async (transaction) => {
        ...
        return this.present(row, undefined, user);
      },
      { isolationLevel: Prisma.TransactionIsolationLevel.Serializable },
    );
  }

  async medicalDischarge(id: string, user: AuthenticatedUser) {
    ...
    return this.prisma.$transaction(async (transaction) => {
      ...
      return this.present(row, approval.createdAt, user);
    });
  }
  ...
  async transfer(id: string, bedId: string, user: AuthenticatedUser) {
    return this.prisma.$transaction(
      async (transaction) => {
  ...
  private present(
    row: HospitalizationRow,
    medicalDischargeApprovedAt: Date | undefined,
    user: AuthenticatedUser,
  ) {
    const financialStatus = this.billingPreview(row, new Date());
    ...
    if (canViewFinancialDetails) {
      return {
        ...
        financialStatus,
        paymentClearance: {
          inOrder: financialStatus.settled,
          status: financialStatus.settled ? 'IN_ORDER' : 'TO_REGULARIZE',
        },
      };
    }

    const { careAuthorization, ...clinicalStay } = row;
    return {
      ...clinicalStay,
      careAuthorization: careAuthorization ? { status: careAuthorization.status } : null,
      medicalDischargeApprovedAt: medicalDischargeApprovedAt?.toISOString() ?? null,
      financialStatus: {
        settled: financialStatus.settled,
        billingMissing: financialStatus.billingMissing,
      },
      paymentClearance: {
        inOrder: financialStatus.settled,
        status: financialStatus.settled ? 'IN_ORDER' : 'TO_REGULARIZE',
